...
export default function CreateHabits({ enableCreate, setEnableCreate, flag, setFlag }){
    ...
    return(
        <>
            <CreateHabit enableCreate={enableCreate}>
                <input onChange={(e) => setHabitName(e.target.value)}type="text" placeholder="nome do hábito" value={habitName} disabled={disabled}></input>
                <WeekDays setHabitDays={setHabitDays} habitDays={habitDays}/>
                <BottomButtons disable={disabled}>
                    <button className="cancel" onClick={() => setEnableCreate(false)} disabled={disabled}>Cancelar</button>
                    <button className="save" onClick={() => saveHabit()} disabled={disabled}>
